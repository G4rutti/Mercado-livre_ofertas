...
def criar_pasta_dia(dia, mes, ano):
            directoryPath = f'/home/davi/Área de Trabalho/Python/Mercado-livre_ofertas/{dia}_{mes}_{ano}'
    ...
    if response.status_code == 200:   
        ...
        if response.url == 'https://www.mercadolivre.com.br/ofertas':
            ...
            def criar_arquivo_ee(dia, mes, ano, page):
                arquivo = Path(f'./{dia}_{mes}_{ano}/ofertas?={dia}_{mes}_{ano}_pt{page}.txt')
            ...
            criar_arquivo_ee(datetime.now().day, datetime.now().month, datetime.now().year, pagina)
            pagina += 1
